...
use router_proxy::ProxyTrait as _;
    ...
    elrond_wasm_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn issue_token(
        ...
        self.token()
            .issue_and_set_all_roles(issue_cost, token_name, token_ticker, 18, None);
    }

    #[only_owner]
    #[endpoint(premint)]
    fn premint(&self, amount: BigUint, to: ManagedAddress) {
    ...
    fn episode_vesting_push(&self, amount: BigUint) {
    ...
    #[only_owner]
    #[endpoint(setRouter)]
    fn set_router(&self, router: ManagedAddress) {
    ...
    fn mint(&self) {
        ...
        require!(
            ...
            "Max supply reached"
        ); // There are 26 episodes in a year, the length of episode_vesting is the number of years of the vesting schedule
        ...
        let to_mint = self
            .episode_vesting()
            .get((episode_number / 26 + 1).try_into().unwrap());

        self.supply().update(|supply| *supply += &to_mint);

        self.token().mint(to_mint.clone());

        self.router_proxy(self.router_contract().get())
            .distribute()
            .add_esdt_token_transfer(self.token().get_token_id(), 0u64, to_mint)
            .transfer_execute();
    }
}
